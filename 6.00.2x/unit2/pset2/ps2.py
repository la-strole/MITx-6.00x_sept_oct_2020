...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        room_X = math.floor(pos.getX())
        room_Y = math.floor(pos.getY())
        if (room_X, room_Y) not in self.cleaned:
            self.cleaned.append((room_X, room_Y))
    ...
    def isPositionInRoom(self, pos):
        ...
        """
        assert isinstance(pos, Position)
        if 0 <= math.floor(pos.getX()) <= self.width - 1 and 0 <= math.floor(pos.getY()) <= self.height - 1:
            return True
        else:
            return False
...
class Robot(object):
    ...
    def setRobotDirection(self, direction):
        ...
        """
        assert type(direction) == int and 0 <= direction < 360
