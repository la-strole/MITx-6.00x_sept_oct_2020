...
def getWordScore(word, n):
    ...
    returns: int >= 0
    """
    assert type(word) == str, 'getWordScore gets word not string type'
    assert word.islower() or len(word) == 0, 'getWordScore gets word in uppercase'
    totalscore = 0
    if len(word) == 0:
        return 0
    else:
        ...
        totalscore *= len(word)
        assert type(totalscore) == int, 'getWordScore type of totalscore is not int '
        if len(word) == n:
...
def updateHand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    assert type(word) == str, 'updateHand word is not string'
    assert type(hand) == dict, 'updateHand hand is not dictionary'
...
def isValidWord(word, hand, wordList):
    ...
    """
    assert type(word) == str, 'isValidWord {} is not string'.format(word)
    assert type(hand) == dict, 'isValidWord {} is not dictionary'.format(hand)
    assert type(wordList) == list, 'isValidWord {} is no a list'.format(wordList)
    hand_copy = hand.copy()
    for letter in word:
        if hand_copy.get(letter, 0) >= 1:
            hand_copy[letter] -= 1
        else:
            return False
    if word in wordList:
